...
# Optional dependency for serial (UART/USB)
try:
...
class _SerialCmdSocket:
    """Socket-like wrapper around pyserial.Serial for minimal code changes.

    Provides recv(), sendall(), settimeout(), close() used by the existing Popoto API.
    """
    ...
    def sendall(self, data: bytes):
        # pyserial write may write fewer bytes; loop until all written
        total = 0
        while total < len(data):
            written = self._ser.write(data[total:])
            if written is None:
                written = 0
            total += written

    def settimeout(self, t: float):
        # pyserial uses 'timeout' in seconds
        self._ser.timeout = t
...
class popoto:
    ...
    def __init__(self, ip='localhost', basePort=17000, logname=None, transport='tcp'):
        ...
        if self.transport == 'tcp':
            ...
            if serial is None:
                raise RuntimeError('pyserial is required for serial transport. Install with: pip install pyserial')
            # In serial mode: ip is the device path (e.g. /dev/ttyUSB0), basePort is baud rate (e.g. 115200)
            self.ser = serial.Serial(ip, int(basePort), timeout=1)
            # Wrap serial as socket-like for existing send/recv code
            self.cmdsocket = _SerialCmdSocket(self.ser)
    ...
    def send(self, message):
        ...
        :type       message:  string
        """
                # Serial transport sends raw pshell/text commands over UART/USB.
        # (Popoto JSON API is socket-based; serial is typically used for pshell/console.)
        if hasattr(self, 'transport') and self.transport == 'serial':
            if message is None:
                return
            ...
            # Helpful aliases so existing scripts keep working.
            _alias = {
                'getVersion': 'version',
                'getversion': 'version',
            }
            ...
            try:
                # pshell generally expects CRLF line endings
                self.cmdsocket.sendall((msg + '\r\n').encode())
            except Exception as e:
                self.logger.error(e)
                self.logger.error("Port:" + str(self.cmdport) + " >>>> " + "SEND ERROR (serial)")
            return
    ...
    def mariaCommand(self, JSmessage):
        ...
        :param      JSmessage:  The Users message
        :type       JSmessage:  string
        """
        ...
        _func_and_args = _func_and_args.split(' ', 1)
        _func = _func_and_args[0]
        _args = _func_and_args[1] if len(_func_and_args) > 1 else ''

        if _args == '':
            message = "{ \"Command\": \"MARIACommand\", \"Destination\": \"" + _dest + "\", \"Function\": \"" + _func + "\"}"
        else:
            message = "{ \"Command\": \"MARIACommand\", \"Destination\": \"" + _dest + "\", \"Function\": \"" + _func + "\", \"Arguments\": " + _args + " }"
        ...
        try:
            ...
            self.logger.err("Invalid JSON message: ", JSmessage)
            ...
            while idx >= 0:
                ...
                if reply:
                    if "Element" in reply:
                        ...
                        #self.logger.info('{}:{}:{}'.format(El['Name'], El['Format'], El['description']))
                    else:
...
# -------------------------------------------------------------------
    def RxCmdLoop(self):
        """Receive loop for command channel.

        TCP: reads CR-terminated JSON messages from the Popoto app socket (original behavior),
             including auto-reconnect if the TCP connection drops.
        Serial: reads CR/LF-terminated text lines from UART/USB and:
             - if a line contains JSON (starts at first '{'), parses and enqueues it
             - otherwise enqueues {'Text': <line>} so callers can still see output
        """
        errorcount = 0
        rxString = ''
        data = b''
        ...
        while (self.is_running == True):
            try:
                ...
                # SERIAL: empty read is typically just a timeout; do NOT treat as disconnect.
                if self.transport == 'serial':
                    ...
                    # Strip ANSI escape codes (pshell may colorize JSON)
                    clean_line = re.sub(r'\x1B\[[0-?]*[ -/]*[@-~]', '', line).strip()

                    # Ignore shell prompt and local echo (common on UART shells)
                    if clean_line.startswith('Popoto->') or clean_line == (self._last_serial_tx or ''):
                        continue


                    # Try to parse one or more JSON objects embedded in the line.
                    decoded_any = False
                    decoder = json.JSONDecoder()
                    pos = clean_line.find("{")
                    while pos != -1:
                        try:
                            reply, endpos = decoder.raw_decode(clean_line, pos)
                            decoded_any = True
                            jsonData = clean_line[pos:endpos]
                            ...
                            if ("RemotePshellCommand" in reply):
                                if (self.remotePshellEnabled):
                                    ...
                                    self.remotePshellCommandQueue.put(reply['RemotePshellCommand'])
                                else:
                            ...
                            # Special-case noisy battery message (keep original behavior)
                            if jsonData == '{"BatteryVoltage":-0.001080}':
                                self.logger.warning("BatteryVoltage reading not valid - likely modem is asleep")

                            self.replyQ.put(reply)

                            # Look for another JSON object later in the line (handles extra trailing text/prompt)
                            pos = clean_line.find("{", endpos)
                        except Exception:
                            # Couldn't decode JSON at this position; skip to next '{' (prevents infinite loop)
                            pos = clean_line.find("{", pos + 1)

                    if not decoded_any:
                        # Non-JSON text line

                        self.replyQ.put({"Text": line})

                    # Logging
                    if (self.verbose > 1):
                        if (self.rawTerminal == False):
                            self.logger.info("\033[1m" + str(line) + "\033[0m")
                        else:
